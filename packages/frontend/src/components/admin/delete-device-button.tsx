...
import { deleteDeviceAction } from "@/app/(dashboard)/admin/actions";
...
export function DeleteDeviceButton({
  ...
  const serialMatches = confirm.trim().toUpperCase() === serial.toUpperCase();
  const canDelete = serialMatches && !isPending;
  ...
  function handleDelete() {
    ...
    formData.set("confirm_serial", confirm);

    startTransition(async () => {
      const result = await deleteDeviceAction(formData);
      if (result.success) {
        handleClose();
        router.refresh();
      } else {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={handleClose} title="Eliminar Equipo">
        <div className="space-y-4">
          <p className="text-sm text-gray-400">
            ...
            junto con todo su historial (telemetria, espectros, alertas y
            mediciones). Esta accion es{" "}
